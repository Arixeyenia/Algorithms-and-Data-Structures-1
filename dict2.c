...
int main(int argc, char **argv){
    ...
    while((read = getline(&keys, &len, searchKeys))!= GETLINE_ERR_OR_EOF){
        ...
        keys[strcspn(keys, "\n")] = 0;
        results = bstSearch(rootNode, SEARCH_PULOCATIONID, keys, results, &counter, &noOfResults);
        printResults(results, SEARCH_PULOCATIONID, keys, &counter, &noOfResults, output);
    }
